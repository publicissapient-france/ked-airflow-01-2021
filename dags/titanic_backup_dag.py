...
with models.DAG('simple_titanic',
    ...
    backup_preprocess = BashOperator(
        ...
        bash_command='gsutil cp gs://' + my_bucket + '/preprocessed_* gs://' + my_bucket + '/backup/')

    backup_train = BashOperator(
        task_id='backup_train',
        bash_command='gsutil cp gs://' + my_bucket + '/model.joblib gs://' + my_bucket + '/backup/')

    backup_predict = BashOperator(
        task_id='backup_predict',
        bash_command='gsutil cp gs://' + my_bucket + '/predict.csv gs://' + my_bucket + '/backup/')
